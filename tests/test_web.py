...
class TestHtmlRoutesNoDb:
    ...
    def test_message_detail_returns_503_without_db(
        self, client: TestClient
    ) -> None:
        response = client.get(
            "/messages/" + "0" * 64, headers={"Accept": "text/html"}
        )
        assert response.status_code == 503
...
class TestHtmlRoutesWithDb:
    ...
    def test_search_with_query(self, client: TestClient) -> None:
        response = client.get(
            "/search?q=hello", headers={"Accept": "text/html"}
        )
        assert response.status_code == 200
    ...
    def test_thread_view_404(self, client: TestClient) -> None:
        response = client.get(
            "/thread/nonexistent", headers={"Accept": "text/html"}
        )
        assert response.status_code == 200
...
class TestRawDownload:
    ...
    def test_raw_content_disposition_is_attachment(
        self, client: TestClient
    ) -> None:
        response = client.get("/raw/" + "0" * 64)
