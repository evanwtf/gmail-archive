...
Not started. Full spec in [plan.md](plan.md#phase-2--synthetic-mbox-fixture-generator).

This is the load-bearing piece and it comes before the parser deliberately:
there is no real mbox export yet, so nothing downstream can be exercised until
the project can generate its own input. It ships as a first-class CLI command,
`gmail-archive gen-fixture OUT --count N --seed S --pathologies ...`, not a test
helper — that is what keeps the README quick start honest.

What it has to produce, each pathology selectable on its own: Gmail label
headers with commas/quotes/unicode/nested paths; missing, malformed, and
timezone-less dates including 1998 and 2087; wrong, absent, and nonexistent
charsets; RFC 2047 encoded-words split mid-multibyte-character; multipart
nesting 5+ deep; duplicate and absent `Message-ID`s; attachments that are
repeated, zero-byte, `../`-named, unicode-named, and 25 MB; bare `From ` lines
inside bodies; base64 with bad padding; 8-bit bytes in headers; and one message
truncated mid-body.

Two tests are non-negotiable, because they are what make the guarantees real
rather than habits:

1. **No generated address may escape the RFC 2606 reserved domains.** Route
   every address through one factory and assert against the generated bytes.
2. **`--seed` must be byte-reproducible.** Generate twice with the same seed and
   compare the files.

Phases 3–10 follow as scoped in [plan.md](plan.md).
